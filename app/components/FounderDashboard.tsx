...
function productBadge(product: string) {
  if (product.includes('Bundle')) return { label: 'Bundle', color: '#4A7C59' }
  if (product.includes('Training')) return { label: 'Training', color: '#6B7FDB' }
  if (product.includes('Nutrition')) return { label: 'Nutrition', color: '#C07850' }
  return { label: product, color: '#555' }
}
...
function OverviewTab({ stats }: { stats: StatsData }) {
  ...
  const totalBreakdown = breakdown.training + breakdown.nutrition + breakdown.bundle || 1
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #E8E4DE', borderRadius: 10, padding: '20px 24px' }}>
        ...
        {[
          { label: 'Training Blueprint', count: breakdown.training, color: '#6B7FDB' },
          { label: 'Nutrition Blueprint', count: breakdown.nutrition, color: '#C07850' },
          { label: 'Full Stack Bundle', count: breakdown.bundle, color: '#4A7C59' },
          ...
          <div key={label} style={{ marginBottom: 14 }}>
            ...
            <div style={{ height: 6, background: '#F0EDE8', borderRadius: 99, overflow: 'hidden' }}>
              <div style={{
                height: '100%',
                width: `${(count / totalBreakdown) * 100}%`,
                background: color,
                borderRadius: 99,
        ...
        {recentSales.slice(0, 10).map((sale) => {
          ...
          return (
            <div key={sale.id} style={{
              ...
              padding: '14px 24px', borderBottom: '1px solid #F2EFE9', gap: 12,
            }}>
...
function OrdersTab({ sales }: { sales: Sale[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #E8E4DE', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: '1fr 1fr 160px 80px 80px',
          padding: '10px 20px',
          background: '#FAFAF8', borderBottom: '1px solid #E8E4DE',
          fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#999',
          gap: 12,
        }}>
          <span>Customer</span>
          <span>Email</span>
        ...
        {filtered.map(sale => {
          ...
          return (
            <div key={sale.id} style={{
              display: 'grid', gridTemplateColumns: '1fr 1fr 160px 80px 80px',
              padding: '12px 20px', borderBottom: '1px solid #F2EFE9',
        ...
        {filtered.length === 0 && (
          <p style={{ padding: '32px 24px', textAlign: 'center', color: '#bbb', fontSize: 14 }}>
            {query ? 'No results' : 'No orders yet'}
          </p>
        )}
      </div>

      <p style={{ fontSize: 12, color: '#bbb', textAlign: 'right' }}>
        Showing {filtered.length} of {sales.length} orders (last 100 from Stripe)
      </p>
    </div>
  )
}
...
type Tab = 'overview' | 'orders' | 'studio'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4A7C59', marginBottom: 4 }}>
              Founder
            </p>
          ...
          <button
            ...
            style={{
              ...
              background: '#E8E4DE', border: 'none', cursor: 'pointer',
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px 32px' }}>
          {tab === 'studio' ? (
            ...
            tab === 'overview'
              ...
              : <OrdersTab sales={stats.recentSales} />
          ) : null}
        </div>
      </div>
    </div>
